...
public class Zadatak1 {
    ...
    public static void main(String[] args) {
        ...
        VideoPlayer videoPlayer = new VideoPlayer(600);

        Control fastForward = new TimeControl(true);

        fastForward.action(videoPlayer);
        fastForward.action(videoPlayer);
        fastForward.action(videoPlayer);
        fastForward.action(videoPlayer);
        fastForward.action(videoPlayer);
        ...
        Control rewind = new TimeControl(false);

        rewind.action(videoPlayer);
        rewind.action(videoPlayer);

        Control volumeUp = new AudioControl(true);

        volumeUp.action(videoPlayer);
        volumeUp.action(videoPlayer);
        volumeUp.action(videoPlayer);
        volumeUp.action(videoPlayer);
        volumeUp.action(videoPlayer);

        Control volumeDown = new AudioControl(false);

        volumeDown.action(videoPlayer);
        volumeDown.action(videoPlayer);

        Control setVideoQuality = new QualityOptimizerControl(60);

        setVideoQuality.action(videoPlayer);
